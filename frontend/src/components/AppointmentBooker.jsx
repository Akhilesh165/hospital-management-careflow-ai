...
const AppointmentBooker = ({ appointments, patients, refreshData }) => {
  ...
  const handleBookAppointment = async (e) => {
    ...
    try {
      ...
      await axios.post('http://127.0.0.1:5000/api/appointments', payload, { headers });
  ...
  const handleUpdateStatus = async (apptId, status) => {
    try {
      await axios.put(`http://127.0.0.1:5000/api/appointments/${apptId}`, { status }, { headers });
      refreshData();
    } catch (err) {
      console.error(err);
  ...
  const handleDeleteAppointment = async (apptId) => {
    ...
    try {
      await axios.delete(`http://127.0.0.1:5000/api/appointments/${apptId}`, { headers });
      refreshData();
    } catch (err) {
      console.error(err);
